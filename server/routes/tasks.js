 ...
 * 
 * Validates the JSON Web Token (JWT) provided in the Authorization header.
 * Extracts the user ID from the decoded token and attaches it to the request.
 */
...
router.get('/tasks', verifyToken, async (req, res) => {
  try {
    ...
    res.json({
      ...
      days: userTasks.days || {},
      upcomingEvents: userTasks.upcomingEvents || []
    });
  } catch (error) {
...
// Update tasks, doneMap, classes, days, and upcomingEvents
router.post('/tasks', verifyToken, async (req, res) => {
  try {
    const { events, doneMap, classes, days, upcomingEvents } = req.body;
    ...
    if (upcomingEvents !== undefined) userTasks.upcomingEvents = upcomingEvents;
